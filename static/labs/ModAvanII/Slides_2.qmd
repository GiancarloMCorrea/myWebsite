...
Si no se incluyen datos, entonces el `use` será 0. Sin embargo, el valor por defecto para `paa` es 1, incluso si no se incluyen datos de composición por edad. Por lo tanto, siempre desactivarlo si es que no se planea usar `paa`.
...
A cada grupo se le entregarán datos simulados.

Objetivo para el día de hoy:

- Explorar los datos
- Discutir enfoques de modelado
