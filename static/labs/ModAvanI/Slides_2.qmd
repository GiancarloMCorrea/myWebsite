...
Usaremos la libería *r4ss* y el modelo de ejemplo que pueden encontrar en [SS Virtual Lab](https://github.com/nmfs-ost/ss3-user-examples/tree/main).
...
-   Explorar el archivo Excel `starter-forecast-helper.xlsx` (`STARTER` sheet) (obtenido de Stock Synthesis Virtual Lab).
...
- Nsamp: esfuerzo de muestreo (e.g., número de embarcaciones muestreadas). Ver @HulsonUnknownTitle2024.
...
-   Explorar el archivo Excel `data-helper.xlsx` (`Data_typical` sheet) (obtenido de Stock Synthesis Virtual Lab).
...
Preparar los datos en formato para SS. Ayudarse con `data-preparation-template.xlsx`.

<br>

Podemos hacerlo en tres principales formas:

1. Modificando el archivo `data.ss` directamente.

2. Utilizando las funciones `SS_readdat` y `SS_writedat` de `r4ss`.

3. Utilizando el [SS GUI](https://github.com/nmfs-ost/ss3-ssi/releases) (interfaz gráfica).

## Preparación de datos

Ejemplo de cómo utilizar `SS_readdat` y `SS_writedat`. 

```{r eval=FALSE}
# Leer el achivo de datos:
data_file = SS_readdat(file = 'ss_models/simple/data.ss')
# Explorar la lista:
data_file$fleetinfo
data_file$catch
data_file$lencomp
# Hacer modificaciones si es necesario
# Volver escribir el archivo de datos:
SS_writedat(datlist = data_file, outfile = 'ss_models/simple/data.ss')
```
...
Es preferible utilizar las funciones `SS_readdat` y `SS_writedat` cuando estemos algo familiarizados con SS. Recomiendo comenzar a aprender SS interactuando directamente con los arhivos  `data.ss` y `control.ss`.
:::
